...
// Approve / Revision / Reject a pending Disbursement, Loan, or Travel
// application from the HR processing screen. This mirrors the old
// hr/process_request_action.php, which was never ported when the app
// moved to Node — until now, every click of Approve/Revision/Reject
// here 404'd and silently did nothing.
router.put('/api/hr/process-action', requireHRAccess, (req, res) => {
    const { id, table, action } = req.body;

    if (!id || !table || !action) {
        return res.status(400).json({ success: false, message: 'Missing required data (id, table, action).' });
    }

    const allowedTables = ['disbursements', 'loans', 'travel'];
    ...
    db.query(query, [action, id], (err, result) => {
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json({ success: false, message: `No record found in '${table}' with that ID.` });
        }
        return res.json({ success: true, message: `Status updated to ${action}` });
    });
});
