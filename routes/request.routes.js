...
// ==========================================================================
// 19b. API ROUTE: GENERATE EXCEL + PDF REPORTS (Pending & Approved, per form type)
...
// Same four columns as the Excel report, laid out as a simple table since
// pdfkit has no built-in table support. Paginates automatically when a
// page's row budget is exhausted.
function writeReportPDF(filePath, formTypeLabel, statusLabel, records) {
    return new Promise((resolve, reject) => {
        ...
        doc.fontSize(16).font('Helvetica-Bold').text(`${formTypeLabel} - ${statusLabel}`);
        doc.moveDown(1);

        const columns = [
            { key: 'employee_id', label: 'Employee ID', width: 110 },
            { key: 'name', label: 'Name', width: 190 },
            { key: 'type', label: 'Type of Form', width: 130 },
        ...
        function drawHeader() {
            ...
            doc.moveDown(0.75);
            doc.moveTo(startX, doc.y).lineTo(x, doc.y).stroke();
            doc.moveDown(0.5);
            doc.font('Helvetica');
        }
        ...
        const rows = records.length === 0
            ? [{ employee_id: 'No records found.', name: '', type: '', date: '' }]
            : records.map(rec => ({
                employee_id: rec.employee_id || '-',
                name: rec.employee_name || '-',
                type: formTypeLabel,
        ...
        rows.forEach(row => {
            ...
            const rowY = doc.y;
            columns.forEach(col => {
                doc.text(String(row[col.key] ?? ''), x, rowY, { width: col.width });
                x += col.width;
            });
            doc.moveDown(0.9);
        });
...
async function generateAllReports() {
    ...
    for (let i = 0; i < formTypes.length; i++) {
        ...
        generatedFiles.push(pendingPath, approvedPath, pendingPdfPath, approvedPdfPath);
    }
