let previousNotiState = {};
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const userId = sessionStorage.getItem('userId') || 'EMP561';
        const username = sessionStorage.getItem('username') || 'Fauzan hensem';
        ...
        document.getElementById('topAvatar').textContent = initials || 'FH';
        ...
        async function fetchDepartmentsFromUsersTable() {
            const tableBody = document.getElementById('deptTableBody');
            const countText = document.getElementById('deptCountText');
            ...
            try {
                ...
                if (result.success && Array.isArray(result.data)) {
                    ...
                    countText.textContent = `${depts.length} department${depts.length !== 1 ? 's' : ''}`;

                    if (depts.length === 0) {
                        tableBody.innerHTML = `<tr><td colspan="4" style="text-align: center; color: #94a3b8; padding: 20px;">No department records found in user profiles.</td></tr>`;
                        return;
                    }
                    ...
                    tableBody.innerHTML = depts.map(d => `
                        <tr>
                            ...
                            <td>${d.headcount}</td>
                            <td>
                                <a href="department-view.html?dept=${encodeURIComponent(d.department)}" class="action-link">View</a>
                                <span class="action-separator">|</span>
                                <a href="department-edit.html?dept=${encodeURIComponent(d.department)}" class="action-link">Edit</a>
                            </td>
                        </tr>
                    `).join('');
                } else {
                    tableBody.innerHTML = `<tr><td colspan="4" style="text-align: center; color: #ef4444; padding: 20px;">Failed to load departments from database.</td></tr>`;
                }
            } catch (err) {
                console.error('Error fetching departments:', err);
                tableBody.innerHTML = `<tr><td colspan="4" style="text-align: center; color: #ef4444; padding: 20px;">Error connecting to Node backend server.</td></tr>`;
            }
        }

        fetchDepartmentsFromUsersTable();
